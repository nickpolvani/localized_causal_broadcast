...
def startProcesses(processes, runscript, hostsFilePath, configFilePath, outputDir):
    ...
    for pid in range(1, processes+1):
        cmd_ext = ['--id', str(pid),
                   '--hosts', hostsFilePath,
                   '--output', os.path.join(outputDirPath, f'{pid}.output'),
                   configFilePath]

        stdoutFd = open(os.path.join(outputDirPath, f'{pid}.stdout'), "w")
        stderrFd = open(os.path.join(outputDirPath, f'{pid}.stderr'), "w")
