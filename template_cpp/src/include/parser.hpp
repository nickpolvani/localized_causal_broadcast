...
public:
  ...
  // check if command line is specified correctly
  void parse() {
    if (!parseInternal()) {
      help(argc, argv);
    }
  ...
  /*
  returns the vector of Hosts, each specified by (process_id, ip_address, port_number_listening),
  defined in hostsFile
  */
...
private:
  ...
  // print message for the user on how to use program
  ...
  // check if ID is specified correctly, and set the id of the process
  bool parseID() {
    if (argc < 3) {
      return false;
    }
  ...
  // check if hosts file path is specified correctly, and set it
  bool parseHostPath() {
    if (argc < 5) {
      return false;
    }
  ...
  // check if output file path is specified correctly, and set it
